...
## §C — A5-S2 accessibility hardening device-only verification (code ships now; lived verification is device-gated)

| ID | Task | Blocked on |
|----|------|-----------|
| A5S2-C1 | **TalkBack full walkthrough — Today screen**: enable TalkBack, open Today with ≥2 DUE doses and ≥1 PRN med. Verify: (a) each DoseCard header announces as ONE node ("Metformin, 500 mg at 8:00 AM, Due now"), (b) state label icon is silent (not announced), (c) Take/Snooze/Skip buttons each include med name in announcement, (d) PRN "Log dose" button includes med name. | Physical device with TalkBack enabled. |
| A5S2-C2 | **TalkBack — MISSED dose**: trigger a dose to go MISSED (or set clock forward past miss window). Open Today. Confirm TalkBack announces "Missed" for that dose card. Confirm the warning icon (WarningAmber) is NOT separately announced. | Physical device with TalkBack enabled. |
| A5S2-C3 | **TalkBack — MedicationList overflow**: open Medications screen, focus the top-right button. Confirm TalkBack announces "More options" (not "Legal"). Activate it; confirm dropdown has Settings, Saved Contacts, and Legal items. | Physical device with TalkBack enabled. |
| A5S2-C4 | **TalkBack — ScheduleSection remove-time button**: add a medication with a Fixed Daily schedule and ≥2 dose times. On the schedule section, focus the "×" chip trailing button. Confirm TalkBack announces "Remove time HH:MM" and the touch target is reachable without precise tapping. | Physical device with TalkBack enabled. |
| A5S2-C5 | **System 2× font + in-app Largest (1.5×) — Today screen**: set device font to Largest + in-app text size to Largest (2.25× combined). Verify: (a) medication name does NOT truncate or clip on any DoseCard, (b) Take/Snooze/Skip buttons remain fully visible and operable (not pushed off-screen), (c) PRN "Log dose" button fully visible, (d) DoseStateLabel (icon + text) wraps if needed but does not clip. | Physical device, Accessibility > Font size > Largest + Settings > Text size > Largest. |
